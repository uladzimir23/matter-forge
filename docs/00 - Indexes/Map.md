---
title: Map of Everything
date: 2026-04-21
---

# Map of Everything

Карта 12 модулей волта. Автообновляется агентом `docs-sync` после
значимых событий.

## 00 Indexes
- [[00 - Indexes/Dashboard]]
- [[00 - Indexes/Map]] ← ты здесь
- [[00 - Indexes/Tags]]

## 10 Vision
- [[10 - Vision/Manifesto]]

## 20 Design Language
- [[20 - Design Language/Metaphor Dictionary]]

## 30 Catalog (0 / 215)

Физические файлы разложены по `src/components/<cat>/`. Catalog-entries
заполняются по мере необходимости.

### bio (21 компонент, 0 записей)
### fluid (14, 0)
### optical (24, 0)
### quantum (9, 0)
### gravity (7, 0)
### thermal (6, 0)
### neural (20, 0)
### kinetic (24, 0)
### atmospheric (5, 0)
### acoustic (11, 0)
### volumetric (11, 0)
### structural (8, 0)
### topology (7, 0)
### state (8, 0)
### molecular (10, 0)
### system (30, 0)

## 40 Architecture
- [[40 - Architecture/FSD]]
- [[40 - Architecture/42 - ADR/ADR-001 Extracted from library]]
- [[40 - Architecture/42 - ADR/ADR-002 Category taxonomy]]
- [[40 - Architecture/42 - ADR/ADR-003 Code-split modes]]
- [[40 - Architecture/42 - ADR/ADR-004 Tailwind stays on CDN]]
- [[40 - Architecture/42 - ADR/ADR-005 GH Pages via Actions]]

## 45 Engineering Workflow
- [[45 - Engineering Workflow/Branching]]
- [[45 - Engineering Workflow/How to add a component]]

## 50 Patterns & Motifs
_пусто_

## 60 Reuse Log
_пусто_

## 70 Session Logs
- [[70 - Session Logs/2026-04-21 Vault + docs-site pipeline]]

## 80 Templates
- [[80 - Templates/component-entry]]
- [[80 - Templates/adr]]
- [[80 - Templates/pattern-essay]]
- [[80 - Templates/reuse-entry]]

## 90 Ideas & Backlog
- [[90 - Ideas & Backlog/Backlog]]

## 95 Attachments
(скрины, gif, видео — не индексируются здесь)
